...
class BaseClientService {
    ...
    protected function configure(array $config = []): void {
        ...
        $this->defaultHeaders = array_merge([
            ...
            'User-Agent' => 'Apps-Auth-Service-Client/1.0',
        ...
        while ($attempts <= $maxRetries) {
            try {
                ...
                if ($statusCode >= 400 && $statusCode < 500) {
                    ...
                    throw new Exception("{$serviceName} Client Error: HTTP {$statusCode}");
                    */
                    return $this->parseResponse($body);
                }
                ...
                throw new Exception("{$serviceName} Server Error: HTTP {$statusCode} after {$maxRetries} retries");
                ...
                throw new Exception("{$serviceName} Connection Failed: " . $e->getMessage() . " after {$maxRetries} attempts");
            } catch (Exception $e) {
    ...
    protected function post(string $uri, array $data = [], array $headers = []): mixed {
    ...
    protected function put(string $uri, array $data = [], array $headers = []): mixed {
    ...
    protected function delete(string $uri, array $data = [], array $headers = []): mixed {
    ...
    protected function patch(string $uri, array $data = [], array $headers = []): mixed {
